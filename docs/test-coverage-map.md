...
現在: **103 GREEN / 4 FAILED** (2026-03-08)
...
| 073 | multi | HTTP_VERSION | **FAIL** | multi-type output |
| 074 | multi_sys | HTTP_VERSION --sys | **FAIL** | multi-type sys |
...
| ID | ケース名 | フィルタ | Status | 必要機能 |
|----|----------|----------|--------|----------|
| 094 | ref_params | IDynamicConceptProviderConcept | **FAIL** | ref param iface |
...
| 107 | rustfmt_25 | POINT --rustfmt max_width=25 | **FAIL** | rustfmt |

## サマリー (2026-03-08)

**`zig build test-red` 結果: 103/107 passed, 4 failed**
...
| Level | 内容 | 現在のカバレッジ |
|-------|------|-----------------|
| Level 1 | WinMDから型/メソッド/フィールドが正しく読めるか | 103/107 |
...
| C: Struct | 14 | 12 | 2 | struct拡張 | T7 (T3依存) |
...
| I: Misc | 7 | 5 | 2 | sort/rustfmt | misc |
| **合計** | **107** | **103** | **4** | | |
...
| ID | 名前 | 原因 |
|----|------|------|
| 073 | multi HTTP_VERSION | 複合型(struct+const)出力未対応 |
| 074 | multi_sys HTTP_VERSION --sys | 同上 sys mode |
| 094 | ref_params | COM iface ref param 未対応 |
| 107 | rustfmt_25 | rustfmt相当の整形未実装 |
