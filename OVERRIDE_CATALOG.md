...
| 7 | `IWindow.get_Content` / `IContentControl.get_Content` | 380-392 | Yes | `(*anyopaque, *?*anyopaque) -> HRESULT` | `!?*IInspectable` | A | BYREF interface pointer. Generic returns `?*anyopaque`; override refines to `?*IInspectable` (nullable, unlike #6). **Must remain hardcoded** (#152): WinRT metadata has no nullable annotation; this is domain knowledge (a window can have no content). windows-rs also uses manual annotations for this. |
...
| 14 | `IXamlMetadataProvider.GetXamlType` / `GetXamlType_2` | 481-499 | No | `(*anyopaque, ?*anyopaque, *?*anyopaque) -> HRESULT` | `!*IXamlType` | B | 2 params: type-name/GUID (in) + IXamlType (BYREF out). Generic non-getter treats both as inputs. **Naming dedup (`GetXamlType_2`) is now generic** (#152): suffix uses `prev_count+1` to match Windows convention (no suffix, `_2`, `_3`, ...). |
